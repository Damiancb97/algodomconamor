  ...
  return (
    <header className="border-b border-[var(--color-pink)]/50 bg-[var(--color-cream)]">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-6 px-4 py-3">
            ...
            <Link
              ...
              className={`text-sm transition ${
                pathname === item.href
                  ...
                  : "text-[var(--color-brown)]/65 hover:text-[var(--color-brown)]"
              }`}
            >
              {item.label}
            </Link>
        ...
        {/* Botones sociales desktop + hamburguesa mobile */}
        <div className="flex items-center gap-3">
          <a
            className="hidden rounded-xl bg-[var(--color-pink)] px-3 py-2 text-sm text-[var(--color-brown)] transition hover:opacity-90 md:inline-flex"
            href={INSTAGRAM_URL}
            target="_blank"
            rel="noreferrer"
          >
          ...
          <a
            className="hidden rounded-xl bg-[var(--color-blue)] px-3 py-2 text-sm text-white transition hover:opacity-90 md:inline-flex"
            href={WHATSAPP_URL}
            target="_blank"
            rel="noreferrer"
          >
          ...
          <button
            ...
            className="flex h-10 w-10 items-center justify-center rounded-xl border border-[var(--color-pink)]/50 bg-white/60 text-[var(--color-brown)] transition hover:bg-white md:hidden"
          >
            {open ? (
      ...
      {/* Panel móvil */}
      {open && (
        <div className="border-t border-[var(--color-pink)]/40 bg-[var(--color-cream)] px-4 pb-5 pt-3 md:hidden">
            ...
            {navItems.map((item) => (
              <Link
                ...
                className={`rounded-xl px-4 py-3 text-sm transition ${
                  pathname === item.href
                    ? "bg-white font-medium text-[var(--color-brown)] shadow-sm"
                    : "text-[var(--color-brown)]/70 hover:bg-white/70 hover:text-[var(--color-brown)]"
                }`}
              >
                {item.label}
              </Link>
          ...
          <div className="mt-4 flex gap-3">
            <a
              ...
              rel="noreferrer"
              onClick={() => setOpen(false)}
              className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-[var(--color-pink)] px-4 py-3 text-sm font-medium text-[var(--color-brown)] transition hover:opacity-90"
            >
            ...
            <a
              ...
              rel="noreferrer"
              onClick={() => setOpen(false)}
              className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-[var(--color-blue)] px-4 py-3 text-sm font-medium text-white transition hover:opacity-90"
            >
              <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
